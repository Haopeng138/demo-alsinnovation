...
import { Calendar, Download, AlertCircle, Zap, TrendingUp, ChevronDown, Search } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900">
      ...
      <main className="p-4">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
            <h1 className="text-2xl font-bold text-blue-900 mb-4 md:mb-0">Climate Forecasts & Business Impact</h1>
            <div className="flex space-x-3">
              <SelectDropdown options={["6 Months", "1 Year", "5 Years"]} value={selectedTimeframe} onChange={setSelectedTimeframe} />
              <SelectDropdown options={["Global", "Americas", "Europe", "Asia"]} value={selectedRegion} onChange={setSelectedRegion} />
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-sm p-4 mb-6">
            <div className="flex justify-between items-center mb-4">
              <h2 className="font-semibold text-blue-900 text-lg">Impacto del clima en KPIs de negocio</h2>
            ...
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={weatherImpactData}>
                  ...
                  <Tooltip />
                  <Legend />
                  <Line type="monotone" dataKey="temp" name="Temperatura (°C)" stroke="#FF8042" strokeWidth={2} />
                  <Line type="monotone" dataKey="production" name="Eficiencia Producción (%)" stroke="#00C49F" strokeWidth={2} />
                  <Line type="monotone" dataKey="logistics" name="Eficiencia Logística (%)" stroke="#FFBB28" strokeWidth={2} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            ...
            <div className="bg-white rounded-lg shadow-sm p-4">
              <h2 className="font-semibold text-blue-900 mb-4">Matriz de riesgo climático</h2>
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                    <CartesianGrid strokeDasharray="3 3" strokeOpacity={0.1} />
                    <XAxis type="number" dataKey="probability" name="Probabilidad" tickFormatter={(val) => `${Math.round(val * 100)}%`} />
                    <YAxis type="number" dataKey="impact" name="Impacto" />
                    <Tooltip formatter={(value, name) => name === "probability" ? [`${(value * 100).toFixed(0)}%`, "Probabilidad"] : [`${value}/100`, "Impacto"]} />
                    <Legend />
                    <Scatter name="Riesgos climáticos" data={climateRiskData} fill="#8884d8">
                      {climateRiskData.map((entry, index) => (
            ...
            {/* Temperature Impact */}
            <div className="bg-white rounded-lg shadow-sm p-4">
              <h2 className="font-semibold text-blue-900 mb-4">Optimizador de procesos por temperatura</h2>
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={temperatureImpactData}>
                    <CartesianGrid strokeDasharray="3 3" strokeOpacity={0.1} />
                    <XAxis dataKey="temp" label={{ value: 'Temperatura (°C)', position: 'bottom' }} />
                    <YAxis domain={[60, 110]} />
                    <Tooltip />
                    <Legend />
                    <Line type="monotone" dataKey="efficiency" name="Eficiencia operativa (%)" stroke="#0088FE" strokeWidth={2} />
                    <Line type="monotone" dataKey="cost" name="Costos relativos (%)" stroke="#FF8042" strokeWidth={2} />
                    <rect x="21%" y="0%" width="15%" height="100%" fill="#00C49F" fillOpacity={0.1} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
              <div className="bg-gray-50 p-3 rounded-lg mt-4 text-xs">
                <span className="font-medium">Recomendación:</span> La zona óptima para operaciones es 20-24°C con eficiencia s95% y costos mínimos.
              </div>
            </div>
          </div>
          ...
          {/* Carbon Emissions */}
          <div className="bg-white rounded-lg shadow-sm p-4 mb-6">
            <h2 className="font-semibold text-blue-900 mb-4">Emisiones de carbono y oportunidades de reducción</h2>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                  ...
                  <Tooltip formatter={(value) => [`${value}%`, "Emisiones"]} />
                </PieChart>
              </ResponsiveContainer>
            </div>
            <div className="mt-4 bg-green-50 p-3 rounded-lg text-xs text-green-700">
              <span className="font-medium">Oportunidad:</span> Potencial para reducir emisiones en un 16% y costos energéticos 
              en un 12% durante los próximos 6 meses mediante ajustes basados en pronósticos climáticos.
            </div>
          </div>
        </div>
      </main>
      ...
      <footer className="bg-blue-900 text-white py-4 text-center text-sm">
        <div className="max-w-7xl mx-auto">© 2025 Arm's Length Services. All rights reserved.</div>
      </footer>
    </div>
  );
}
